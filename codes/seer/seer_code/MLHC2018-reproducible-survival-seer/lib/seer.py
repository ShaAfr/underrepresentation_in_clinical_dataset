...
def parse_specification(seer_specification_path):
    ...
    with open(seer_specification_path) as specification:
        for line in specification:
            ...
            if len(line_entries) > 6 and line_entries[0] == '@':
...
def parse_incidences(seer_specification_path, seer_incidences_path):
    ...
    # get killed when reading the whole file
    ...
    #data = pd.read_fwf(seer_incidences_path, colspecs=colspecs, header=None, names=descriptions)
    #print("step 1 done")
    #data = data.apply(pd.to_numeric, errors='coerce').fillna(-1).astype(np.int32)

    # try break the file into chunks, starting with 10,000 lines a chunk
    reader = pd.read_fwf(seer_incidences_path, colspecs=colspecs, header=None, names=descriptions, chunksize=10000)
    data = pd.concat([r for r in reader], ignore_index=True)
    data = data.apply(pd.to_numeric, errors='coerce').fillna(-1).astype(np.int32)
